...
local NP = LibStub("AceAddon-3.0"):NewAddon("OldschoolUINameplates")

-- Optional class-power resource trackers are provided by the ResourceBars
-- module (not yet ported). Until then these stay inert so the optional class-
-- power display can never error; ResourceBars will define the real ones later.
OldschoolUI.GetSoulFragments    = OldschoolUI.GetSoulFragments    or function() return nil end
OldschoolUI.GetMaelstromWeapon  = OldschoolUI.GetMaelstromWeapon  or function() return nil end
OldschoolUI.GetTipOfTheSpear    = OldschoolUI.GetTipOfTheSpear    or function() return nil end
OldschoolUI.GetWhirlwindStacks  = OldschoolUI.GetWhirlwindStacks  or function() return nil end
OldschoolUI.HandleTipOfTheSpear   = OldschoolUI.HandleTipOfTheSpear   or function() end
OldschoolUI.HandleWhirlwindStacks = OldschoolUI.HandleWhirlwindStacks or function() end
...
local function GetFont()
    if OldschoolUI and OldschoolUI.GetFontPath then
        return OldschoolUI.GetFontPath("nameplates")
    end
    return (p and p.font) or defaults.font
end
local function GetNPOutline()
    return (OldschoolUI and OldschoolUI.GetFontOutlineFlag and OldschoolUI.GetFontOutlineFlag("nameplates")) or "OUTLINE"
end
local function GetNPUseShadow()
    return not OldschoolUI or not OldschoolUI.GetFontUseShadow or OldschoolUI.GetFontUseShadow("nameplates")
end
...
ns.NP_ABSORB_STYLE_TEX = {
    blizzard = "Interface\\AddOns\\OldschoolUI\\media\\textures\\shields\\blizzard-nameplates.png",
    striped  = "Interface\\AddOns\\OldschoolUI\\media\\textures\\shields\\striped3.tga",
...
local defaults = {
    ...
    showEnemyPets = false,
    font = "Interface\\AddOns\\OldschoolUI\\media\\fonts\\Expressway.TTF",
...
do
    local TB = "Interface\\AddOns\\OldschoolUI\\media\\textures\\"
...
local function ApplyHealthBarTexture(plate)
    ...
    local path   = OldschoolUI.ResolveTexturePath(ns.healthBarTextures, texKey, "Interface\\Buttons\\WHITE8x8")
...
-------------------------------------------------------------------------------
--  Glow Engines provided by shared OldschoolUI_Glows.lua
--  Local aliases for the pandemic glow wrapper below.
-------------------------------------------------------------------------------
local _G_Glows = OldschoolUI.Glows
...
-------------------------------------------------------------------------------
local GLOW_TEX = "Interface\\AddOns\\OUI_Nameplates\\Media\\background.png"
...
ns.TARGET_ARROW_DIR = "Interface\\AddOns\\OUI_Nameplates\\Media\\Arrows\\"
...
local function EnsureFocusOverlay(plate)
    ...
    local STRIPE_TEX = "Interface\\AddOns\\OUI_Nameplates\\Media\\striped-v2.png"
...
local frameCache = CreateFramePool("Frame", UIParent, nil, nil, false, function(plate)
    ...
    local function AddBorder(parent)
        local PP = OldschoolUI and OldschoolUI.PP
        if PP then
    ...
    plate.castSpark:SetTexture("Interface\\AddOns\\OldschoolUI\\media\\cast_spark.tga")
    ...
    plate.castShield:SetTexture("Interface\\AddOns\\OUI_Nameplates\\Media\\shield.png")
...
-- Exposed for the cast overlay file (OldschoolUINameplates_CastOverlay.lua)
...
local function SetupAuraCVars()
    ...
    if SetCVar then
        ...
        -- Friendly player CVars are only written when OUI is managing
        -- friendly player nameplates. When the user disables the "Show OUI
...
-- Resolve class/power color from OUI global system.
...
local function GetClassPipColor(classFile, powerKey)
    if OldschoolUI then
        if powerKey then
            ...
            local c = OldschoolUI.GetPowerColor and OldschoolUI.GetPowerColor(key)
            if c then return { c.r, c.g, c.b } end
        end
        local rc = OldschoolUI.GetResourceColor and OldschoolUI.GetResourceColor(classFile)
        if rc then return { rc.r, rc.g, rc.b } end
        local cc = OldschoolUI.GetClassColor and OldschoolUI.GetClassColor(classFile)
        if cc then return { cc.r, cc.g, cc.b } end
    end
...
local function UpdateClassPowerOnPlate(plate)
    ...
    if classPowerType == "SOUL_FRAGMENTS_DEVOURER" then
        ...
        local bar = plate._cpBar
        local cur, maxC = 0, 50
        if OldschoolUI and OldschoolUI.GetSoulFragments then
            cur, maxC = OldschoolUI.GetSoulFragments()
    ...
    elseif classPowerType == "MAELSTROM_WEAPON" then
        cur, maxP = OldschoolUI.GetMaelstromWeapon()
    elseif classPowerType == "TIP_OF_THE_SPEAR" then
        cur, maxP = OldschoolUI.GetTipOfTheSpear()
    elseif classPowerType == "WHIRLWIND_STACKS" then
        cur, maxP = OldschoolUI.GetWhirlwindStacks()
...
local function EnableClassPowerWatcher()
    ...
    if type(classPowerType) == "string" then
        ...
        -- so tracking works even without OldschoolUIResourceBars loaded.
        ...
        classPowerWatcher:SetScript("OnEvent", function(_, event, ...)
            ...
            elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
                ...
                -- Skip if OldschoolUIResourceBars is loaded (it handles routing).
                if _G._ERB_AceDB then
                    RefreshClassPower()
                    return
                end
                local unit, castGUID, spellID = ...
                if unit == "player" and OldschoolUI then
                    if OldschoolUI.HandleTipOfTheSpear then
                        OldschoolUI.HandleTipOfTheSpear(event, unit, castGUID, spellID)
                    end
                    if OldschoolUI.HandleWhirlwindStacks then
                        OldschoolUI.HandleWhirlwindStacks(event, unit, castGUID, spellID)
                    end
                end
                RefreshClassPower()
            elseif event == "PLAYER_DEAD" or event == "PLAYER_ALIVE" then
                if not _G._ERB_AceDB and OldschoolUI then
                    if OldschoolUI.HandleTipOfTheSpear then
                        OldschoolUI.HandleTipOfTheSpear(event)
                    end
                    if OldschoolUI.HandleWhirlwindStacks then
                        OldschoolUI.HandleWhirlwindStacks(event)
                    end
                end
                RefreshClassPower()
            elseif event == "PLAYER_REGEN_ENABLED" then
                if not _G._ERB_AceDB and OldschoolUI and OldschoolUI.HandleWhirlwindStacks then
                    OldschoolUI.HandleWhirlwindStacks(event)
                end
                RefreshClassPower()
            else
...
local function RefreshThreatCache()
    ...
    if role == "NONE" and GetSpecialization and GetSpecializationRole then
...
local function HideBlizzardFrame(nameplate, unit)
    ...
    -- Suppress unconditionally -- if we're called, an OUI plate is taking
    ...
    if not hookedUFs[uf] then
        ...
        hooksecurefunc(uf, "SetAlpha", function(self)
            ...
            -- Only force alpha 0 while an OUI plate owns this nameplate.
            -- When the nameplate is recycled for a friendly unit, the OUI
...
function NameplateFrame:UpdateHealthColor()
    ...
    if focusTex ~= "none" and UnitIsUnit(unit, "focus") then
        ...
        local MEDIA = "Interface\\AddOns\\OUI_Nameplates\\Media\\"
...
function NameplateFrame:UpdateClassification()
    ...
    if ns.IsQuestMob and ns.IsQuestMob(self.unit) then
        OldschoolUI.SafeAtlas(self.class, "Crosshair_Quest_64")
    else
        local c = UnitClassification(self.unit)
        if c == "elite" or c == "worldboss" then
            OldschoolUI.SafeAtlas(self.class, "nameplates-icon-elite-gold", "Interface\\Tooltips\\EliteNameplateIcon")
        elseif c == "rareelite" then
            OldschoolUI.SafeAtlas(self.class, "nameplates-icon-elite-silver", "Interface\\Tooltips\\EliteNameplateIcon")
        elseif c == "rare" then
            OldschoolUI.SafeAtlas(self.class, "nameplates-icon-rareelite", "Interface\\Tooltips\\RareEliteNameplateIcon")
        else
            self.classFrame:Hide()
            self:UpdateNameWidth()
            return
...
function NameplateFrame:UpdateImportantCastGlow(spellID)
    ...
    local Glows = _G_Glows or OldschoolUI.Glows
...
function NameplateFrame:ShowInterrupted(interrupterGUID)
    ...
    self.castName:SetText(OldschoolUI.L("Interrupted"))
...
do
    ...
    specLoginFrame:SetScript("OnEvent", function(_, event, unit)
        ...
        if OldschoolUI and OldschoolUI._specSwitchRegistry
           and #OldschoolUI._specSwitchRegistry > 0 then
            return
        end
...
function NP:OnInitialize()
    NP.db = LibStub("AceDB-3.0"):New("OldschoolUINameplatesDB", { profile = defaults })
    p = NP.db.profile
    ns.db = NP.db
    -- Append SharedMedia textures to runtime tables so SM texture keys resolve at runtime
    if OldschoolUI.AppendSharedMediaTextures then
        OldschoolUI.AppendSharedMediaTextures(
            ns.healthBarTextureNames,
            ns.healthBarTextureOrder,
            nil,
            ns.healthBarTextures
