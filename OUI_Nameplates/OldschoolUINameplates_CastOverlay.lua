-------------------------------------------------------------------------------
--  OldschoolUINameplates_CastOverlay.lua
--
...
--  does. Mirrors the block in OldschoolUINameplates.lua at the layout
...
local function BuildOverlay()
    ...
    shield:SetAllPoints()
    shield:SetTexture("Interface\\AddOns\\OUI_Nameplates\\Media\\shield.png")
...
local function ConfigureImportantCastGlow(ov, spellID)
    ...
    if not enabled or not ov.glowHost then
        if ov._glowActive then
            local Glows = _G.OldschoolUI and _G.OldschoolUI.Glows
...
SlashCmdList["EUIOVERLAYTEST"] = function()
    ...
    if not _testOverlay then
        ...
        _testOverlay.name:SetText(OldschoolUI.L("Test Spell"))
