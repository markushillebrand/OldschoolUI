...
local function buildSoundVis(page)
    page:AddRow(OUI.Widgets.Dropdown(page, {
...
local function buildReminders(page)
    local a = ABR()
...
OUI:RegisterModule("OUI_AuraBuffReminders", {
    ...
    tabs = {
        { title = "Display",            build = buildDisplay },
        { title = "Sound & Visibility", build = buildSoundVis },
        { title = "Reminders",          build = buildReminders },
    },
})
