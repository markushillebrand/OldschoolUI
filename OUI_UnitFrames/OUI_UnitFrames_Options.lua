...
-- boss1-5 share one config block; everything else is per unit. A single unit
-- picker (see ALL_UNITS below) drives the Frames tab.
...
-- ---- per-unit row block (shared by Main and Mini tabs) ---------------------
local function UnitRows(page, o)
...
local UNIT_PICK_VALUES, UNIT_PICK_ORDER, UNIT_BY_KEY = {}, {}, {}
for _, o in ipairs(ALL_UNITS) do
    UNIT_PICK_VALUES[o.key] = o.label
    UNIT_PICK_ORDER[#UNIT_PICK_ORDER + 1] = o.key
    UNIT_BY_KEY[o.key] = o
end
...
local function buildGeneral(page)
    ...
    page:AddRow(OUI.Widgets.Toggle(page, {
        ...
        set = function(v) if UF then UF.db.profile.hideBlizzard = v; UF:HideBlizzardFrames() end end,
    }))
    page:AddRow(OUI.Widgets.Toggle(page, {
...
local function buildFrames(page)
    page:AddRow(OUI.Widgets.Dropdown(page, {
        label = "Unit", values = UNIT_PICK_VALUES, order = UNIT_PICK_ORDER,
        tooltip = "Pick a frame to configure; its settings appear below.",
        get = function() return selectedUnit end,
        set = function(v) selectedUnit = v; if OUI.RefreshOptionsBody then OUI.RefreshOptionsBody() end end,
    }))
    UnitRows(page, UNIT_BY_KEY[selectedUnit] or ALL_UNITS[1])
end

local function buildBuffs(page)
    page:AddRow(OUI.Widgets.Toggle(page, {
...
OUI:RegisterModule("OUI_UnitFrames", {
    ...
    tabs = {
        ...
        { title = "Frames",       build = buildFrames },
        { title = "Player Buffs", build = buildBuffs },
    },
})
